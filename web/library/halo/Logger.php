...
class Logger
{
    ...
    /* class method */
    private static function write ( $level, &$domain, &$info , $file = '', $line='' ,$file='')
    {
        $logLevel = HaloEnv::logLevel();
        if ( $level < $logLevel )
        {
    ...
    public static function loggerFileName ($domain, $ext='log')
    {
        ...
        $rootDir = self::$LOG_ROOT_DIR;
        if(empty(self::$LOG_ROOT_DIR))
        {
            $rootDir = APPLICATION_PATH.'/../logs';
        }
        
        $filepath = $rootDir.'/'.$_ENV['APP_NAME'].'/'.$date.'/';
    ...
    /* instance method */
    public function __DEBUG__ ( $info , $output='file'  )
    {
        $traces = debug_backtrace(0);
        $invoker = $traces[0];
        Logger::write(HALO_LOG_DEBUG, $this->_domain, $info , $invoker['file'], $invoker['line'] ,$output );
    }

    public function __INFO__ ( $info , $output='file' )
    {
        $traces = debug_backtrace(0);
        $invoker = $traces[0];
        Logger::write( HALO_LOG_INFO, $this->_domain, $info , $invoker['file'], $invoker['line'] ,$output );
    }

    public function __WARNNING__ ( $info ,$output='file' )
    {
        $traces = debug_backtrace(0);
        $invoker = $traces[0];
        Logger::write( HALO_LOG_WARNNING, $this->_domain, $info , $invoker['file'], $invoker['line'] ,$output);
    }

    public function __ERROR__ ( $info ,$output='file' )
    {
        $traces = debug_backtrace(0);
        $invoker = $traces[0];
        Logger::write( HALO_LOG_EEROR ,$this->_domain,$info , $invoker['file'], $invoker['line'] ,$output );
    }

    public function __TRACKER__ ( $info ,$output='file' )
    {
        $traces = debug_backtrace(0);
        $invoker = $traces[0];
        Logger::write( HALO_LOG_TRACKER ,$this->_domain, $info , $invoker['file'], $invoker['line'] ,$output );
    }
...
//                            YafDebug::dump($str);
                        ...
                        if ($pagetime >= 3)
                        {
                            Logger::traceUser(0, 3001,  array('time'=>$pagetime*1000));
                        }
                    }
                }
                elseif($cmd == 'url')
