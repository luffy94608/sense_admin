...
class HaloMongo
{
     ...
     * @return Builder
     */
    public function orWhereLike($field = '', $value = '', $flags = 'i', $enableStartWildcard = true, $enableEndWildcard = true)
    {
        $field = (string) trim($field);
        $value = (string) trim($value);
        $value = quotemeta($value);
...
//                    $this->_querySafety => true,
    ...
    /**
     * 执行 mapreduce 返回的结果集 by 鹿秀飞
     * @param $result
     * @param bool|false $returnCursor
     * @return $this|array|MongoCursor
     * @throws Exception
     */
    public function selectCollection($result,$returnCursor = false)
    {
        try {
            $cursor = $this->_dbhandle->selectCollection($result)->find();
            ...
            throw new Exception('MongoDB selectCollection failed to execute: ' .
                $Exception->getMessage());
            // @codeCoverageIgnoreEnd
        }
    }
