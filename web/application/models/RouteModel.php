...
class RouteModel extends Halo_Model
{

    private $web;
    private $web_slave;
    public $tbl_name = 'routes';
    ...
    public function updateParams($id,$params)
    {
        if(!empty($params))
        {
            ...
            $subTmpResult=$this->db->getResultsByCondition($this->tbl_name,HaloPdo::condition('parent_id=?',$id));
            if($subTmpResult)
            {
            ...
            foreach($params as $param)
            {
                ...
                $tmpItem['parent_id']=$id;

                if(!empty($param['id']) && array_key_exists($param['id'],$subResultMap))
                {
                    $updateIds[]=$tmpId;
                    if(!($name==$subResultMap[$tmpId]['name'] && $sortNum==$subResultMap[$tmpId]['sort_num']))
                    {
                        $tmpItem['id']= $tmpId;
                        $updateData[]=$tmpItem;
                    }
            ...
            if(count($updateData))
            {
                $this->db->batchUpdateData($this->tbl_name,array_keys($updateData[0]),$updateData,"name=VALUES(name),sort_num=VALUES(sort_num)");
            }
            if(count($insertData))
            {
                $this->db->batchInsertData($this->tbl_name,array_keys($insertData[0]),$insertData);
            }
            $deleteIds=array_diff($existIds,$updateIds);
            if(count($deleteIds))
            {
                $this->db->delRowByCondition2($this->tbl_name,sprintf('id IN (%s)',implode(',',$deleteIds)));
            }
    ...
    public function delete($id)
    {
        ...
        if($result)
        {
            $this->db->delRowByCondition2($this->tbl_name,HaloPdo::condition('parent_id = ',$id));
        }
        return $result;
    }
    ...
    public function addExtraInfo(&$result)
    {
        ...
        $paramsResult = $this->web_slave->getResultsByCondition($this->tbl_name,sprintf('parent_id IN (%s) ORDER BY sort_num ASC',implode(',',$lockIds)));
        $paramsMap = [];
        if($paramsResult)
        {
