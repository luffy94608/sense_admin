...
class StringUtil
{
     ...
     */
    public static function  hasPrefix($str, $prefix, $ignoreCase = false)
    {
        if($ignoreCase)
            $ret = stripos($str, $prefix);
        else
            $ret = strpos($str, $prefix);

        if(gettype($ret) == 'integer' && $ret == 0)
        {
            return true;
        }
        return false;
     ...
     */
    public static function hasSuffix($str, $end, $ignoreCase = false)
    {
        if($ignoreCase)
            $ret = stripos($str, $end);
        else
        ...
        if(gettype($ret) == 'integer' )
            return $ret + strlen($end) == strlen($str);
