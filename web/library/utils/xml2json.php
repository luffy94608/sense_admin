...
// 		echo is_array($simpleXmlElementObject);
       		...
       		foreach($simpleXmlElementObject as $key=>$value) {
  	     		...
  	     		if((is_string($key)) && ($key == SIMPLE_XML_ELEMENT_OBJECT_PROPERTY_FOR_ATTRIBUTES)) 
  	     		{
  	     			foreach($value as $key1 => $value1)
  	     			{
  	     				...
  	     				{
  	     					if(!is_array($resultArray[$key1]) || !isset($resultArray[0]))
  	     					{
  	     						$v = $resultArray[$key1];
  	     						$resultArray[$key1] = array();
  	     						$resultArray[$key1][] = $v;
  	     					}
  	     					$resultArray[$key1][] = xml2json::convertSimpleXmlElementObjectIntoArray($value1, $recursionDepth);
  	     				}
  	     				else
  	     				{
  	     					$resultArray[$key1] = xml2json::convertSimpleXmlElementObjectIntoArray($value1, $recursionDepth);
				...
				if(isset($resultArray[$key]))
				{
					if(!is_array($resultArray[$key]) || !isset($resultArray[0]))
					{
						$v = $resultArray[$key];
